...
public class WebOrdersSteps {
    ...
    public void user_validates_total_is_calculated_properly_for_quantity(Integer quantity) {
        ...
        int discountAmountInt = Integer.parseInt(discountAmount);//making String discount as Integer discount to perform math calculation
        int pricePerUnitInt = Integer.parseInt(pricePerUnit);
        if (discountAmountInt == 0) {
            expectedTotal = quantity * pricePerUnitInt; //8*100=800 no discount
        } else {
            expectedTotal = quantity * pricePerUnitInt;
            expectedTotal = expectedTotal - expectedTotal * discountAmountInt / 100; //10*100=920 with discount
            //1000-(1000*/100)
            //100*10=800 -> 800/100=
        }
